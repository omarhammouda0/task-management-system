...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query("SELECT c FROM Comment c WHERE c.taskId = :taskId AND c.status <> 'DELETED'")
    ...
    @Query("SELECT c FROM Comment c WHERE c.userId = :userId AND c.status <> 'DELETED'")
    Page<Comment> findByUserIdAndNotDeleted(@Param("userId") Long userId, Pageable pageable);

    @Query("SELECT c FROM Comment c WHERE c.createdBy = :userId AND c.status <> 'DELETED'")
